...
#### Reflection (as well as issues, challenges & resolutions)
The biggest challenge I had when implementing the primer was making it case insensitive, when I made the line lower case then the final display will show the information in lower case. So I created a 'lowerLine' string variable to store a copy of 'line' and do the matching using the lowercase version but using the original 'line' to store the matched line into the vector. However, I kept failing at creating the copy. After a lot of research I realised the solution was very simple, I had forgotten to write: 'lowerLine = line;'. That was silly but I was very pleased I could moved on.

Another thing to take into account is that my code reads the file twice, and for that is important to clear the stream status and reset the position to the beginning of the file, so when we want to read the file again from the beginning is ready to do so. This was an issue when doing the primer6 (my original solution only read the file once), so I used it here when I was refactoring and decided to do the count of lines in a different step.
...
This challenge was divided in two parts. First part was to create threads and join them, and the second part was to create threads and detach them. 
...
#### Reflection (as well as issues, challenges & resolutions)
Although, I learnt and practice a lot from implementing two solutions, the main reflection here is start coding when you are sure you understand what is been asked. One of my favourite parts of coding using OOP is overloading the constructors. It is also cool, to just get/set the methods/properties of an object using a '.' for example myTriangle.setBase() or myTriangle.getBase().

---
## Section 2 - Programming Paradigms
### Programming Paradigms
Patterns are everywhere, even when trying to solve a problem and in computer science programmers have seen those patterns in the way they approach a problem, this is what it is called a programming paradigm. 
...
Lets take a look at the concepts of five of them:

#### **Imperative:** 
Imperative programming focusses on how a programme operates.  Two of most widely used paradigms are procedural and objected oriented.
...
#### Functional:
"Functional programming is a declarative type of programming style that treats computation as the evaluation of mathematical functions." Esample: Haskell https://deepsource.io/glossary/functional-programming/

There are two main paradigms of programming under which others can be claissified, the imperative and the declarative. I found this funny explanation of the difference between Imperative and declarative paradigms and as it gave me a good image to keep in my head I want to sahre it :
...
Having this knowledge then allows the person to learn a given programming language faster as the person knows what is looking for, for example to write a loop, by understanding the basics the person knows what are the elements required, all is left is to check the syntax. 
